...
class spidercalendarModelbigcalendar extends JModel
...
function getelementindexinarray($array , $element)
{
...

	{
		...
		if(!JRequest::getVar('calendar'))
			$calendar=0;
		if($cat_ids!='')
			$db->setQuery( "SELECT #__spidercalendar_event.*,#__spidercalendar_event_category.color  from #__spidercalendar_event JOIN #__spidercalendar_event_category ON #__spidercalendar_event.category = #__spidercalendar_event_category.id where #__spidercalendar_event_category.published=1 and #__spidercalendar_event.category IN (".$cat_ids.") and #__spidercalendar_event.published=1 and ( ( (date<='".$db->getEscaped(substr( $date,0,7))."-01' or date like '".$db->getEscaped(substr( $date,0,7))."%') and  (date_end>='".$db->escape(substr( $date,0,7))."-01' ) or date_end='0000-00-00'  ) or ( date_end is Null and date like '".$db->getEscaped(substr( $date,0,7))."%' ) ) and calendar='".$db->getEscaped($calendar)."' ORDER BY #__spidercalendar_event.time ASC  ");
		else
			$db->setQuery( "SELECT *  from #__spidercalendar_event  where  published=1 and ( ( (date<='".$db->getEscaped(substr( $date,0,7))."-01' or date like '".$db->getEscaped(substr( $date,0,7))."%') and  (date_end>='".$db->escape(substr( $date,0,7))."-01' ) or date_end='0000-00-00'  ) or ( date_end is Null and date like '".$db->getEscaped(substr( $date,0,7))."%' ) ) and calendar='".$db->getEscaped($calendar)."' ORDER BY #__spidercalendar_event.time ASC ");
...
//echo	"SELECT date,date_end,text_for_date from #__spidercalendar where published=1 and ( ( (date<='".substr( $date,0,7)."-01' or date like '".substr( $date,0,7)."%') and  date_end>='".substr( $date,0,7)."-01' ) or ( date_end is Null and date like '".substr( $date,0,7)."%' ) )  ";
		...
		$rows = $db->loadAssocList();
			...
			//echo "SELECT * from #__spidercalendar_event where published=1 and ( ( (date<='".$db->getEscaped(substr( $date,0,7))."-01' or date like '".$db->getEscaped(substr( $date,0,7))."%')  ) or ( date_end is Null and date like '".$db->getEscaped(substr( $date,0,7))."%' ) ) and calendar='".$db->getEscaped($calendar)."'  ";
			$id_array=array();
